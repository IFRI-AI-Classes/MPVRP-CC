...
async def submit_solutions_endpoint(
    ...
    from backend.paths import TEMP_DIR

    TEMP_DIR.mkdir(parents=True, exist_ok=True)
    temp_path = str(TEMP_DIR / unique_filename)
    ...
    if not result.get("ok", False):
        raise HTTPException(status_code=400, detail=result.get("processor_info", "Evaluation failed"))
